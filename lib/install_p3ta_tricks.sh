...
#
# Portalgun deliberately does not execute the upstream install.sh. This module
# owns dependency installation, immutable staging, service lifecycle, rollback,
# health checks, and registry state so the knowledge service remains isolated
# from the shared pentest environment.
...
_p3ta_validate_configuration() {
    if [ "${PORTALGUN_P3TA_TRICKS_ALLOW_UNSAFE_ROOT:-0}" != "1" ]; then
        ...
        _p3ta_die "p3ta-tricks root must be absolute"
        return 1
    fi
    ...
    if ! [[ "$PORTALGUN_P3TA_TRICKS_SERVICE" =~ ^[A-Za-z0-9_.@-]+\.service$ ]]; then
        _p3ta_die "invalid p3ta-tricks service name"
        return 1
    fi
    ...
    if ! [[ "$PORTALGUN_P3TA_TRICKS_MIN_PAGES" =~ ^[0-9]+$ ]] ||
        [ "$PORTALGUN_P3TA_TRICKS_MIN_PAGES" -lt 1 ]
    then
        _p3ta_die "invalid minimum page count"
        return 1
    fi
}
...
for candidate in root.rglob("*"):
    ...
    if candidate.is_symlink():
        try:
            ...
            raise SystemExit(
                f"invalid symbolic link {candidate.relative_to(root)}: {exc}"
            )

        if resolved != root and root not in resolved.parents:
            raise SystemExit(
                "symbolic link escapes checkout: "
    ...
    if not (stat.S_ISREG(mode) or stat.S_ISDIR(mode)):
        raise SystemExit(
            f"unsupported checkout entry: {candidate.relative_to(root)}"
        )
...
_p3ta_stage_release() {
    ...
    if [ -d "$source_root/vendor" ] &&
        find "$source_root/vendor" -mindepth 1 -print -quit | grep -q .
    then
        "$venv_root/bin/python3" -m pip install \
            --disable-pip-version-check \
            --no-input \
            --no-index \
            --find-links "$source_root/vendor" \
...
PY
    ...
    "$venv_root/bin/python3" -m pip freeze \
        > "$stage_root/python-packages.txt"
...
_p3ta_unit_path() {
    printf '%s/%s' \
        "$PORTALGUN_P3TA_TRICKS_SYSTEMD_DIR" \
...
install_p3ta_tricks() (
    ...
    local unit_path
    local unit_backup=""
    local previous_install=0
    local new_install_active=0
    local committed=0
    ...
    cleanup() {
        ...
        if [ "$committed" -ne 1 ] && [ "$new_install_active" -eq 1 ]; then
            systemctl stop "$PORTALGUN_P3TA_TRICKS_SERVICE" \
                >/dev/null 2>&1 || true
            ...
            if [ "$previous_install" -eq 1 ] && [ -d "$previous_root" ]; then
                ...
                systemctl disable "$PORTALGUN_P3TA_TRICKS_SERVICE" \
                    >/dev/null 2>&1 || true
            fi

            if [ -n "$unit_backup" ] && [ -f "$unit_backup" ]; then
                mv -f "$unit_backup" "$unit_path"
            elif [ "$previous_install" -eq 0 ]; then
                rm -f "$unit_path"
            fi
    ...
    _p3ta_success \
        "p3ta-tricks ready on port $PORTALGUN_P3TA_TRICKS_PORT ($page_count pages, commit ${resolved_commit:0:12})"
)
...
verify_p3ta_tricks() {
    ...
    recorded_commit=$(jq -r '.resolved_commit // empty' \
        "$PORTALGUN_P3TA_TRICKS_REGISTRY")
    recorded_pages=$(jq -r '.processed_page_count // 0' \
        "$PORTALGUN_P3TA_TRICKS_REGISTRY")
    ...
    [ "$page_count" = "$recorded_pages" ] || {
        _p3ta_die "p3ta-tricks page count differs from registry state"
        return 1
    }

    jq -e \
        --arg root "$PORTALGUN_P3TA_TRICKS_ROOT" \
        --arg service "$PORTALGUN_P3TA_TRICKS_SERVICE" \
        --arg user "$PORTALGUN_P3TA_TRICKS_USER" \
        --argjson port "$PORTALGUN_P3TA_TRICKS_PORT" \
        '.status == "complete" and
         .offline_mode == true and
         .install_root == $root and
         .service == $service and
         .service_user == $user and
         .port == $port' \
        "$PORTALGUN_P3TA_TRICKS_REGISTRY" >/dev/null || {
            _p3ta_die "p3ta-tricks registry record is inconsistent"
            return 1
        }

    systemctl is-enabled --quiet "$PORTALGUN_P3TA_TRICKS_SERVICE" || {
        _p3ta_die "p3ta-tricks service is not enabled"
        return 1
    }
    systemctl is-active --quiet "$PORTALGUN_P3TA_TRICKS_SERVICE" || {
        _p3ta_die "p3ta-tricks service is not active"
        return 1
    }
    _p3ta_health_check || {
        _p3ta_die "p3ta-tricks HTTP health check failed"
        return 1
    }
...
verify_p3ta_tricks_section() {
    ...
    if [ -d "$source_root" ] &&
        page_count=$(_p3ta_validate_checkout "$source_root" 2>/dev/null)
    then
        ...
        _p3ta_row "$FAIL_MARK" "p3ta-tricks content" "missing, unsafe, or incomplete"
        failures=$((failures + 1))
    fi
    ...
    if [ -x "$PORTALGUN_P3TA_TRICKS_ROOT/venv/bin/gunicorn" ]; then
        ...
        _p3ta_row "$FAIL_MARK" "p3ta-tricks runtime" "isolated environment missing"
        failures=$((failures + 1))
    fi

    if systemctl is-enabled --quiet "$PORTALGUN_P3TA_TRICKS_SERVICE" 2>/dev/null &&
        systemctl is-active --quiet "$PORTALGUN_P3TA_TRICKS_SERVICE" 2>/dev/null
    then
        _p3ta_row "$PASS_MARK" "p3ta-tricks service" "enabled and active on port $PORTALGUN_P3TA_TRICKS_PORT"
    else
        _p3ta_row "$FAIL_MARK" "p3ta-tricks service" "not enabled and active"
        failures=$((failures + 1))
    fi
    ...
    if _p3ta_health_check 2>/dev/null; then
        ...
        _p3ta_row "$FAIL_MARK" "p3ta-tricks HTTP" "health check failed"
        failures=$((failures + 1))
    fi

    if [ -s "$PORTALGUN_P3TA_TRICKS_REGISTRY" ] &&
        jq -e '.status == "complete" and .offline_mode == true' \
            "$PORTALGUN_P3TA_TRICKS_REGISTRY" >/dev/null 2>&1
    then
        _p3ta_row "$PASS_MARK" "p3ta-tricks registry" "complete and offline"
    else
        _p3ta_row "$FAIL_MARK" "p3ta-tricks registry" "missing or inconsistent"
        failures=$((failures + 1))
    fi
